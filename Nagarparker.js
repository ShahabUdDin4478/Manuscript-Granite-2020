var aster = ee.ImageCollection("ASTER/AST_L1T_003"),
    ...
    roi = /* color: #d63000 */ee.Geometry.MultiPoint(
        [[70.75520845467393, 24.330488553828978],
         [70.76825471932237, 24.31703613881554]]),
    nangarparker = 
    /* color: #d63000 */
    /* shown: false */
    /* displayProperties: [
    ...
    ee.Geometry.Polygon(
        [[[70.65831542884219, 24.40212430635539],
          [70.65831542884219, 24.303600198523917],
          [70.87563873206484, 24.303600198523917],
          [70.87563873206484, 24.40212430635539]]], null, false);
...
//print(sentImage)
var sentImage = sentImage.median().clip(nangarparker);
...
var region = nangarparker;
var DSregion = nangarparker;
...
var landImage = landCollection.clip(nangarparker);
var landbands = ['B5','B6','B7','B10','B11'];

//Filter and load ASTER Image
var asterImage = ee.Image('ASTER/AST_L1T_003/20070330060231');
//print("ASTER Scene", asterImage);
var asterImage = asterImage.clip(nangarparker);
...
var eigenCollection = ee.Array([[0],[1],[2],[3],[4],[5]]);
var tscale = 5;
...
var DSLand = decStr(landBandsImage, nangarparker, 1000);
var DSaster = decStr(asterBandsImage, nangarparker, 1000);
var DSsent = decStr(sentBandsImage, nangarparker, 1000);
...
Map.addLayer(pcImageS, {bands: ['pc1', 'pc2', 'pc4'], min: -2, max: 2}, 'Sentinel 2 - PCA  124');
...
*/
var h =ee.Image.cat([pcImageADS.select('pc5').multiply(1.50),pcImageS.select('pc2').multiply(0.5),pcImageSDS.select('pc5').multiply(1)])
Map.addLayer(h,{min: -3, max: 3},'FCCs combination 1')
/*var g =ee.Image.cat([pcImageL.select('pc2').multiply(1.50),pcImageL.select('pc1').multiply(0.375),pcImageS.select('pc5').multiply(1.125)])
Map.addLayer(g,{min: -3, max: 3},'FCCs combination 3')
*/
/*var g =ee.Image.cat([pcImageL.select('pc2'), pcImageA.select('pc1'),pcImageS.select('pc2')])
Map.addLayer(g,{min: -3, max: 3},'FCCs combination 3')*/
/*var g =ee.Image.cat([pcImageL.select('pc2'), pcImageA.select('pc1'),pcImageS.select('pc2')])
Map.addLayer(g,{min: -3, max: 3},'FCCs combination 2')
*/
//SDS-5, ADS-5, LDS-3,4, A-1, S-2, L-1,2
var g =ee.Image.cat([pcImageL.select('pc1'), pcImageADS.select('pc5'),pcImageSDS.select('pc5')])
Map.addLayer(g,{min: -3, max: 3},'FCCs combination 3')
var g =ee.Image.cat([pcImageL.select('pc2'), pcImageADS.select('pc5'),pcImageSDS.select('pc5')])
Map.addLayer(g,{min: -3, max: 3},'FCCs combination 3')
/*Export.image.toDrive({
  image: g,
...
Map.setCenter(70.75520845467393,24.330488553828978, 12);
//no data for aster on the eastern side and no data for sentinel on the northern side in nangarparker
